...
export default function ResultModal({
    ...
    const diamond = gameAssets.find((asset) => asset.name === 'diamond')?.quantity ?? 0;
    const hint = gameAssets.find((asset) => asset.name === 'hint')?.quantity ?? 0;
    const life = gameAssets.find((asset) => asset.name === 'life')?.quantity ?? 0;

    useEffect(() => {
        if (!visible || userInfo.id == null) {
            return;
        }
        ...
        syncAsset({
            ...
            console.error('ResultModal asset sync failed:', error);
        });
                    ...
                    >
                        ...
                        {isCorrect && (
                            <View className="flex-col gap-0 px-3 py-2 bg-accent/20 rounded-xl">
                                <Text className="text-sm font-mono">Rewards</Text>
                        ...
                        {imageAnswer && (
                            <View className="items-center gap-1 w-full">
                                <View className="flex-row items-center justify-center gap-5 w-full">
                                    <Text className="text-xl font-medium">{bugtong.answer}</Text>
                                </View>
                                <Text className="text-center italic text-lg px-6 font-light leading-none mb-2">{bugtong.question}</Text>
                                <Image
                                    source={imageAnswer}
